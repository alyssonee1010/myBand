...
function getSetlistCacheMetadataKey(groupId, setlistId) {
    return `${groupId}:${setlistId}`;
}
function getContentCacheUrl(groupId, contentId) {
...
export function getCacheableSetlistItems(items) {
    return items.filter((item) => Boolean(item.content.fileUrl));
}
export async function getCachedContentFile(groupId, contentId) {
...
export async function getSetlistCacheStatus(groupId, setlistId, contentIds) {
    if (!isSetlistCacheSupported()) {
        return {
            supported: false,
...
export async function cacheSetlistFiles(groupId, setlistId, items, fetchContentFile, onProgress) {
    if (!isSetlistCacheSupported()) {
    ...
    const cache = await window.caches.open(SETLIST_CACHE_NAME);
    await Promise.all(staleContentIds.map((contentId) => cache.delete(getContentCacheUrl(groupId, contentId))));
    let completed = 0;
...
export async function clearSetlistCache(groupId, setlistId, contentIds) {
    if (!isSetlistCacheSupported()) {
        return;
    }
    const metadata = await readCacheMetadata();
    ...
    const cache = await window.caches.open(SETLIST_CACHE_NAME);
    await Promise.all(cachedContentIds.map((contentId) => cache.delete(getContentCacheUrl(groupId, contentId))));
    delete metadata[metadataKey];
    await writeCacheMetadata(metadata);
}
//# sourceMappingURL=setlistCache.js.map